...
//  Public domain.
//
//  Host multiprecision for BI-MUL / BI-DIVMOD / BI-ISQRT (T16Forth lineage).
...
enum BigIntHost {
    ...
    private static func biWriteLimbs(_ bi: Int, sign: Int, limbs: [Int64]) {
        ...
        let cap = biCap(bi)
        if ls.count > cap {
            // T16Forth throws here. We must not zero the destination (that made PI. print 0).
